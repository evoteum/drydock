...
def real_kubectl_apply(kubeconfig: str, manifest_path: str) -> None:
    """
    ...
    kubeconfig: path to the kubeconfig file created by kubeadm init
    manifest_path: path to a file or directory containing Kubernetes manifests
    """

    if not os.path.exists(manifest_path):
        raise KubectlApplyError(
            f"Manifest path not found: {manifest_path}"
        )
